...
if (!isset($_GET['user_id']) || empty($_GET['user_id'])) {
    echo '<div style="color:red;text-align:center;margin:2em;">No user selected.</div>';
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['profile_pic'])) {
        ...
        } else if ($uploadedFile['error'] != UPLOAD_ERR_NO_FILE) {
    ...
    if (empty($firstName) || empty($lastName)) {
        ...
        if (isset($result) && $result) {
            ...
            $messageType = "success";
            header("Location: U_Manage.php?msg=profile_updated");
            exit();
        } elseif ($stmt !== null) {
...
if ($stmt) {
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        echo "<div style='color:red;text-align:center;margin:2em;'>User not found.</div>";
        exit();
    }
    mysqli_stmt_close($stmt);
} else {
    echo "<div style='color:red;text-align:center;margin:2em;'>Error fetching user data.</div>";
    exit();
}
?>
<!DOCTYPE html>
        ...
        <div class="profile-container profile-complete">
            ...
            <form class="profile-form" method="POST" action="" enctype="multipart/form-data">
                ...
                <div class="form-actions">
                    <button type="button" class="btn-secondary" onclick="window.location.href='U_Manage.php'">Back to User Management</button>
